...
class brulage
{
	...
	function brulage($ref_action)
	{
		global $gCms;
		$db= cmsms()->GetDb();
		$query = "SELECT cp.genid, cp.ref_action, cp.ref_equipe  FROM ".cms_db_prefix()."module_compositions_compos_equipes AS cp, ".cms_db_prefix()."module_compositions_equipes AS eq WHERE cp.ref_equipe = eq.ref_equipe AND cp.ref_action = ? ORDER BY eq.numero_equipe ASC";
